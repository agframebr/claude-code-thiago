// PROMPT VERBATIM — extraído de WF01 "Agente IA Vendedora"
// Placeholders n8n ({{ $('Info').item.json.tarefa.* }}, {{ $now.format('ffff') }}) substituídos por template literal TS.
import type { TarefaKanban, FunilKanban } from '../../../tipos.ts';
import { agora } from '../../../lib/datas.ts';

export interface OpcoesPromptMaria {
  tarefa: TarefaKanban | null;
  funil: FunilKanban | null;
}

export function buildPromptMaria(opts: OpcoesPromptMaria): string {
  const tarefa = opts.tarefa;
  const etapas = opts.funil?.steps ?? tarefa?.board?.steps ?? [];
  const etapasDesc = etapas.map((s) => `${s.name}: ${s.id}`).join(', ');
  const nomeEtapa = tarefa?.board_step?.name ?? '(sem etapa)';
  const idEtapa = tarefa?.board_step?.id ?? '(sem id)';
...
Observações: [se houver]"
  ...
  <ferramenta id="Atualizar_tarefa">
    ...
    **Regras**:
      ...
      * IDs das etapas disponíveis via: ${etapasDesc}
...
<tarefa-atual>
  ...
  * **Etapa atual**: ${nomeEtapa} (ID: ${idEtapa})
  * **Título atual**: ${titulo}
